...
if (-not [string]::IsNullOrWhiteSpace($ConfigFile)) {
	...
	try {
		$script:IHConfig = Get-Content $ConfigFile -Raw | ConvertFrom-Json
	} catch {
		Write-Error "[Insight Harbor] ConfigFile is not valid JSON: $($_.Exception.Message)"
		exit 1
	}
	# Apply values only if caller did not explicitly pass the corresponding parameter
	if (-not $PSBoundParameters.ContainsKey('TenantId') -and $script:IHConfig.auth.tenantId) {
		$TenantId = $script:IHConfig.auth.tenantId; $script:TenantId = $TenantId
	}
	if (-not $PSBoundParameters.ContainsKey('ClientId') -and $script:IHConfig.auth.clientId) {
		$ClientId = $script:IHConfig.auth.clientId; $script:ClientId = $ClientId
	}
	if (-not $PSBoundParameters.ContainsKey('ClientSecret') -and $script:IHConfig.auth.clientSecret) {
		$ClientSecret = $script:IHConfig.auth.clientSecret; $script:ClientSecret = $ClientSecret
	}
	if (-not $PSBoundParameters.ContainsKey('ClientCertificateThumbprint') -and
		-not [string]::IsNullOrWhiteSpace($script:IHConfig.auth.certificateThumbprint)) {
		$ClientCertificateThumbprint = $script:IHConfig.auth.certificateThumbprint
		$script:ClientCertificateThumbprint = $ClientCertificateThumbprint
	}
	if (-not $PSBoundParameters.ContainsKey('OutputPath') -and $script:IHConfig.ingestion.outputLocalPath) {
		$OutputPath = $script:IHConfig.ingestion.outputLocalPath
	}
}

# ============================================================
# [INSIGHT HARBOR] MOD 4 — RAW-ONLY EXPLOSION GUARD
# ============================================================
if ($script:IHConfig -and $script:IHConfig.pax.explosionMode -eq 'raw') {
	if ($ExplodeArrays -or $ExplodeDeep) {
		Write-Warning "WARNING [Insight Harbor]: explosionMode is 'raw' in config. -ExplodeArrays and -ExplodeDeep are disabled. Explosion is handled downstream by the Python processor. Proceeding in RAW mode."
	}
...
'@
	...
	if ($script:IHConfig -and -not $ExportWorkbook) {
		...
		# Collect CSV files to process (per-activity split or single combined file)
		$ihCsvFiles = @()
		if ($script:CsvSplitFiles -and $script:CsvSplitFiles.Count -gt 0) {
			$ihCsvFiles = @($script:CsvSplitFiles | Where-Object { Test-Path $_ })
		} elseif ($OutputFile -and (Test-Path $OutputFile) -and $OutputFile -match '\.csv$') {
		...
		foreach ($ihCsvPath in $ihCsvFiles) {
			...
			try {
				...
				$ihMeta = [ordered]@{
					...
					runTimestampUtc      = if ($script:metrics -and $script:metrics.StartTime) { $script:metrics.StartTime.ToUniversalTime().ToString('o') } else { $ihRunDate.ToString('o') }
					startDate            = if ($StartDate) { $StartDate } else { $null }
					endDate              = if ($EndDate)   { $EndDate }   else { $null }
					activityTypes        = @($ActivityTypes)
					recordCount          = [int]$script:metrics.TotalStructuredRows
